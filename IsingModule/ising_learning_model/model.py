...
class ModelResults:
    ...
    def __init__(self, results, results_samples, runtime, final_loss, results_test=None, results_samples_test=None, batch_times=None):
...
class Model(ABC):
    ...
    def _sample_bulk_threaded(self, thetas: torch.Tensor, ys: torch.Tensor) -> tuple[list, list, list]:
        losses_bulk = []
        configurations_bulk = []
        energies_bulk = []
    ...
    def train(
        ...
        self._lmd = self.lmd_init_value
        eta_gamma = self.settings.learning_rate_gamma
        eta_lmd = self.settings.learning_rate_lmd
        eta_offset = self.settings.learning_rate_offset
        eta_theta = self.settings.learning_rate_theta
        ...
        try:
            for epoch in range(self.settings.optim_steps):
                eta_gamma *= self.settings.dacay_rate
                if self._verbose:
                ...
                for idx_mini_batch, (thetas, ys, indices) in enumerate(
                    ...
                    (
                        ...
                        energies_bulk,
                        configurations_bulk,
                    ) = self._sample_bulk_threaded(thetas, ys)
                ...
                #    
                #    for f, y, c in zip(energies_bulk, ys.tolist(), configurations_bulk):
                #        print("predizione: ",f)
                #        print("target: ",y)
                #        print("offset(epsilon): ",self._offset)
        ...
        return ModelResults(
            results, results_samples, t_total, results["loss"].iloc[-1],
            results_test, results_samples_test, batch_times
        )
