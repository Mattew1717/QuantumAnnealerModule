"""
XOR comparison FullIsingModule vs ModularNetwork.
Entry points: run_xor_2d (single 2D run, executed as __main__) and
compare_xor_models_all_dimensions (full 1D-6D sweep).
"""
...
def get_xor_params():
    ...
    return {
        ...
        'profile': os.environ['DWAVE_PROFILE'],  # used only when annealer_type=quantum
...
def train_full_ising_model(dim, X_train, y_train, X_test, y_test, params,
    ...
    model = FullIsingModule(
        ...
        profile=params['profile'],          # ignored unless annealer_type=quantum
        num_reads=params['num_reads'],      # ignored unless annealer_type=quantum
    ).to(params['device'])
